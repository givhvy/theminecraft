...
import { tileBaseColor } from './textures';
...
interface Particle { x: number; y: number; z: number; vx: number; vy: number; vz: number; r: number; g: number; b: number; life: number }
const particles: Particle[] = [];

export function spawnParticle(x: number, y: number, z: number, vx: number, vy: number, vz: number, r: number, g: number, b: number, life: number): void {
  if (particles.length >= MAX_PARTICLES) particles.shift();
  particles.push({ x, y, z, vx, vy, vz, r, g, b, life });
}
export function spawnBlockParticles(bx: number, by: number, bz: number, blockId: number): void {
